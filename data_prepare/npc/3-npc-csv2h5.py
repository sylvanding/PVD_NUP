...
# -*- coding: utf-8 -*-
"""
3-npc-csv2h5.py: 将NPC样本归一化并保存为H5格式

功能:
1. 读取processed/{subfolder}/csv/目录下的NPC样本
...
def main():
    parser = argparse.ArgumentParser(description='将NPC样本CSV转换为H5格式')
    ...
    # 设置路径
    input_dir = os.path.join(args.output_dir, args.subfolder, "csv")
    output_path = os.path.join(args.output_dir, args.subfolder, f"npc_{args.subfolder}.h5")
    
    # 设置日志
    logger = setup_logging()
    
    logger.info("=" * 60)
    logger.info("开始将NPC样本CSV转换为H5格式...")
    logger.info("=" * 60)
