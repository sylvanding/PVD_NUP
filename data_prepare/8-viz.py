...
# -*- coding: utf-8 -*-
"""
8-viz.py: 可视化H5转换后的CSV点云

功能:
1. 读取7-h5tocsv-csv/目录下的csv文件
2. 生成2D可视化图像
3. 保存到8-h5tocsv-viz-png/目录
"""
...
def visualize_single_file(csv_path: str, output_dir: str) -> str:
    ...
    visualize_pointcloud_2d(
        ...
        point_size=1.0,
        cmap='viridis',
        title=f"{name_without_ext} (from H5, {len(points)} points)",
        dpi=100
    )
...
def main():
    ...
    INPUT_DIR = os.path.join(DATA_ROOT, "7-h5tocsv-csv")
    OUTPUT_DIR = os.path.join(DATA_ROOT, "8-h5tocsv-viz-png")
    
    # 设置日志
    logger = setup_logging()
    
    logger.info("开始可视化H5转换后的CSV点云...")
