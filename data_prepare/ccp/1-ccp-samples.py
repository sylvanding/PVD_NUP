...
# -*- coding: utf-8 -*-
"""
1-ccp-samples.py: 处理CCP点云样本

功能:
1. 读取指定发育时期的CCP点云CSV文件
2. 过滤点数不足的样本
3. 对点数过多的样本使用最远点采样
4. 归一化到坐标原点（平移）
5. 输出到outputs/{stage}/csv/目录
"""
...
def load_ccp_pointcloud(csv_path: str,
                       coord_cols: List[str] = CCP_COORD_COLS) -> np.ndarray:
    """
    加载CCP点云
...
def process_ccp_sample(points: np.ndarray,
                       target_points: int,
                       min_points: int,
...
def process_stage(input_dir: str,
                  output_dir: str,
                  stage: str,
    ...
    stage_output_dir = os.path.join(output_dir, stage, "csv")
    
    if not os.path.exists(stage_input_dir):
        logger.error(f"输入目录不存在: {stage_input_dir}")
        return {}
    
    Path(stage_output_dir).mkdir(parents=True, exist_ok=True)
    
    # 获取所有CSV文件
    csv_files = sorted(glob(os.path.join(stage_input_dir, "*.csv")))
    logger.info(f"[{stage}] 找到 {len(csv_files)} 个CCP点云文件")
    ...
    for csv_path in csv_files:
        ...
        try:
            points = load_ccp_pointcloud(csv_path)
            ...
            output_path = os.path.join(stage_output_dir, f"{name_without_ext}.csv")
            ...
            if stats['samples_processed'] % 100 == 0:
                logger.info(f"  [{stage}] 已处理: {stats['samples_processed']}/{len(csv_files)}")
...
def main():
    parser = argparse.ArgumentParser(description='处理CCP点云样本')
    ...
    logger.info("=" * 60)
    logger.info("开始处理CCP点云样本...")
    logger.info("=" * 60)
    logger.info(f"发育时期: {args.stage}")
    logger.info(f"输入目录: {args.input_dir}/{args.stage}")
    logger.info(f"输出目录: {args.output_dir}/{args.stage}/csv")
    ...
    # 保存统计信息
    stats_path = os.path.join(args.output_dir, args.stage, "extraction_stats.json")
